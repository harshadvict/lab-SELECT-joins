...
select role.name,user_new.name from role inner join user_new on role.id=user_new.role_id;
...
select user_new.name,role.name from user_new left join role on user_new.role_id=role.id;
...
select user_new.name, skill_new.name from user_new
inner join role on user_new.role_id = role.id
inner join profile on user_new.profile_id = profile.id
inner join skill_new on profile.id = skill_new.id
where role.name = 'Alumini';

select * from profile;
select * from skill_new;
...
inner join role on user_new.role_id=role.id
inner join profile on user_new.profile_id=profile.id
inner join experience on user_new.profile_id=experience.profile_id
where role.name='alumini';
